...
export function ComponentAttr() {
				...
				return (
					<Upload
						...
						onChange={(info) => {
							...
							if (file) {
								reader.onload = (e) => {
									const modelPath = e.target?.result;
									if (modelPath && curComponentId) {
							...
							// 更新模型文件路径
							// if (file && curComponentId && curComponent) {
							//   updateComponentProps(curComponentId, {
							//     modelPath: file.url,
							//   });
							// }
	...
	function valueChange(changeValues: any) {
		if (curComponent?.name === 'Line' && curComponentId) {
			...
			updateBasicAttributes(curComponentId, changeValues);
		} else if (curComponent?.name === 'ModelViewer' && curComponentId) {
			// const fileObj = changeValues.modelPath.file;
